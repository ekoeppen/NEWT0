...
void PkgReserveRelocations(newtRefArg package, pkg_stream_t *pkg)
{
	...
	if (ix >= 0) {
		...
		uint32_t num_pages = 1 + ((htonl(relocations[num_relocations]) - htonl(relocations[1])) / 1024);
	...
	if (ix >= 0) {
		...
		for (size_t i = 0; i < num_relocations; i++) {
			...
			if (pkg_reloc / 1024 != page_number) {
				...
				PkgWriteData(pkg, pkg->header_size + pkg->var_data_size + sizeof(header),
						&reloc_set, sizeof(reloc_set));
				PkgWriteData(pkg, pkg->header_size + pkg->var_data_size + sizeof(header) + sizeof(reloc_set),
